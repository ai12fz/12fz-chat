...
func (d *DB) GetAgent(ctx context.Context, botID string) (*Agent, error) {
	...
	err := d.pool.QueryRow(ctx,
		"SELECT id, bot_id, display_name, model, system_prompt, capabilities, status, api_key, created_at, updated_at FROM chat.agents capabilities, status, api_key, api_url, merchant_id FROM chat.agents WHERE bot_id = $1",
		botID,
