...
def generate_reply(content, force_reply=False, history=None):
    """Use hermes CLI to decide relevance + generate reply"""
    role_desc = BOT_ROLES.get(BOT_ID, "")
    env = {**os.environ, "HERMES_HOME": HERMES_HOME}

    # Build conversation context from history — smart filtering
    context = ""
    if history:
        # Filter: keep substantive messages, skip noise
        ctx_lines = []
        noise_words = {"嗯", "嗯嗯", "好的", "ok", "okay", "收到", "来了", "好的好的",
                       "明白了", "知道", "知道了", "是的", "对", "好", "行", "可以",
                       "没问题", "了解", "哦", "噢", "啊", "哈哈", "呵呵",
                       "HEARTBEAT_OK"}
        for h in history:
            text = h['text'].strip()
            # Skip noise/single-word acknowledgments
            if text.lower() in noise_words or len(text) <= 2:
                continue
            # Skip bot "ignored" messages
            if h.get("is_bot") and ("ignored" in text.lower() or "不相关" in text):
                continue
            who = "你" if h.get("is_bot") else h.get("sender", "用户")
            ctx_lines.append(f"{who}: {text[:200]}")
        # Take last 30 from filtered
        ctx_lines = ctx_lines[-30:]
        if ctx_lines:
            context = "\n".join(ctx_lines) + "\n\n"
...
last_msg_ids = set()
group_history = defaultdict(lambda: deque(maxlen=100))  # per-group message history
...
def on_message(ws, raw):
    try:
        ...
        # Save user message to group history
        group_history[gid].append({"sender": s, "text": content, "is_bot": False})
        ...
        reply = generate_reply(content, force_reply=mentioned, history=list(group_history[gid]))
        if reply:
            # Save bot's own reply to history
            group_history[gid].append({"sender": BOT_ID, "text": reply, "is_bot": True})
